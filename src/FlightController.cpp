...
FlightController::FlightController(const FlightController &flightController): pitch(flightController.pitch), roll(flightController.roll), yaw(flightController.yaw) {}

FlightController::FlightController(float pitch, float roll, float yaw): pitch(pitch), roll(roll), yaw(yaw) {}
...
FlightController FlightController::fromJson(cJSON *json) {
    ...
    if (!cJSON_IsNumber(pitchJson) || !cJSON_IsNumber(rollJson) || !cJSON_IsNumber(yawJson)) {
